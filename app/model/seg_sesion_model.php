...
class SegSesionModel {
		...
		public function add($data) {
			try {
				...
				if($sesion != 0) {
					...
					return $this->response->SetResponse(true, 'Id del registro: '.$sesion);
				} else { 
					return $this->response->SetResponse(false, 'No se inserto el registro'); 
				}
			} catch(\PDOException $ex) {
				$this->response->errors = $ex;
				return $this->response->SetResponse(false, 'catch: Add model seg_sesion');
			}
		}
		...
		public function edit($data, $id) {
			...
			try {
				...
				$this->response->errors = $ex;
				return $this->response->SetResponse(false, 'catch: Edit model seg_sesion');
			}
		}
